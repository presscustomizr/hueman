...
(function (api, $, _) {
  $.extend( HUBaseControlMethods, {
    ...
    /// NORMALIZE URLS
    //////////////////////////////////////////////////
    // normalizeURL : function( _url ) {
    //     var _original_url = _url,
    //         _strip;
    ...
    //     if ( -1 != _url.indexOf( 'mailto:' ) ) {
    //         _strip = ['%0d', '%0a', '%0D', '%0A'];
    //         _url = this._deep_replace(_strip, _url);
    //     }
    ...
    //      */
    //     if ( -1 != _url.indexOf(':') && ! in_array( _url[0], array( '/', '#', '?' ) ) &&
    //         ! preg_match('/^[a-z0-9-]+?\.php/i', _url) )
    //         _url = 'http://' + _url;

    //     return _url;
    // },

    // function _deep_replace( _search, _subject ) {
    //   var _subject = _subject.toString(),
    //       _count = 1;


    //   while ( _count ) {
    //     _subject = this._str_replace( _search, '', _subject, _count );
    //   }
  ...
  $.extend( HUDynamicMethods, {
    ...
    initialize: function( id, options ) {
      ...
      this.view_event_map = [
        ...
        // //add classes to the edit and trash icon button
        // {
        //   trigger   : 'click keydown cancel_alert',
        //   selector  : [ '.' + options.params.css_attr.display_alert_btn, '.' + options.params.css_attr.edit_view_btn ].join(','),
        //   name      : 'edit_view',
        //   actions   : 'toggleHeaderActiveClasses'
        // },
        //view content setup
        {
        ...
        //   name      : 'edit_view',
        //   actions   : ['renderViewContent', 'toggleViewVisibility']
        // }
      ...
      this.view_content_event_map = [
        //set input value
        {
          trigger   : 'propertychange change click keyup input paste colorpickerchange',//colorpickerchange is a custom colorpicker event @see method setupColorPicker => otherwise we don't
      ...
      this.control_event_map = [
        ...
        //renders the model's view when a new model is added
        //@param = model {}
        {
          trigger   : 'model_added',
          name      : 'render_view',
      ...
      //declares an empty colllection array
      this.collection = [];
      ...
      //mostly used to set and get the current expansion status of a given view
    ...
    ready : function() {
      ...
      api.bind( 'ready', function() {
        ...
        //populate the collection on init
        control.fetchSavedCollection();
        //bind 'collection_update'
...
(function (api, $, _) {
  $.extend( HUDynamicMethods, {
    ...
    addModel : function( obj, key ) {
      //Do we have a model?
      //If yes, does this model have an id?
      // => if not, then _initNewModel
      ...
      //control.updateCollection( model, key, true );
      this.container.trigger( 'model_added', { model : model , dom_event : obj.dom_event } );

      //if a model is manually added, trigger a specific event
    ...
    updateModel : function( obj ) {
      ...
      //say it loud and clear to the controller
    ...
    //fired on 'model_updated'
    //The idea is to send only the currently modified model instead of the entire collection
    //the entire collection is sent anyway on api(setId).set( value ) on 'collection_updated', and accessible in the preview via api(setId).bind( fn( to) )
...
(function (api, $, _) {
  $.extend( HUDynamicMethods, {
    ...
    //!! HAS TO BE FIRED AFTER THE SAVED MODELS HAVE BEEN ADDED TO THE COLLECTION => otherwise an unwanted collection update will be triggered when adding the saved models
    //each collection update will refresh the setting
    ...
    updateCollection : function( obj, key ) {
      ...
      if ( _.has( obj, 'collection' ) ) {
        ...
        //say it
        //Important note : this event refreshes the customizer setting value/
        //It's not listened to before the api is ready => the collection update on startup won't trigger and change setting
...
(function (api, $, _) {
  $.extend( HUDynamicMethods, {
    ...
    renderView : function( obj ) {
      ...
      //create the view value and update its expanded status
      control.hu_views.create(model.id);
      control.hu_views(model.id).set('closed');

      //add a state listener on change
      ...
      //Emit the view_rendered event => this will fire the setup emitters and listener for this view
      ...
      //if the view has been manually added, then trigger another event
    ...
    renderViewContent : function( obj ) {
      //if the view has already been rendered, stop here
    ...
    //normalize the way a view element visibility is handled
    //adds the class 'open' when the view element is visible
    //@param : $_view => the view element
    //@param : selector of the view element to toggle
    makeSortable : function(obj) {
      ...
      $( '.' + control.css_attr.views_wrapper, control.container ).sortable( {
          ...
          update: function( event, ui ) {
            //the event "sortupdate" could have been used here instead of trigger a custom one
      ...
      // if ( ! $('.' + control.css_attr.view_header, control.getViewEl(obj.model.id) ).find('.hu-move-icon').length )
      //   $('.' + control.css_attr.view_header, control.getViewEl(obj.model.id) )
      //       .prepend( $('<div/>', {class:'hu-move-icon'}).append( $('<span/>', { class: 'fa fa-arrows-alt'}) ) );
    },
    ...
    setViewVisibility : function(obj) {
      ...
      //when a new view is manually added, the event is not flagged "isTrigger" by jQuery.
      //=> we can identify it to open the newly added view
      // if ( "view_rendered:view_event_map" == obj.event.trigger && ! _.has( obj.dom_event, 'isTrigger' ) )
      //   this.hu_views(obj.model.id).set( 'expanded' );
      // else if ( "view_rendered:view_event_map" != obj.event.trigger )
      //   this.hu_views(obj.model.id).set( 'expanded' == control.hu_views(obj.model.id).get() ? 'closed' : 'expanded' );

      //if the view has been expanded by a triggered event, typically when a new model is added, then activate the edit button
      // if ( 'expanded' == control.hu_views(obj.model.id).get() ) {
      //   control.toggleHeaderActiveClasses( obj, $('.' + control.css_attr.edit_view_btn, control.getViewEl(obj.model.id ) ) );
      // }
    },
    ...
    //callback of hu_views() instance on change
    ...
    detectInputChange : function( obj ) {
      ...
      //check if the value is different than the one in the collection.
      //if not return false => will break the chain of actions (if any)
      //the changed value to check depends on the event name
...
(function (api, $, _) {
  $.extend( HUDynamicMethods, {
    ...
    //////////////////////////////////////////////////
    //on view rendered
    //view_rendered:view_event_map => get splitted to extract the event map name
    setupViewListeners : function(obj) {
      //get the event map to setup from the event trigger
    ...
    setupListeners : function( event_map , obj ) {
      ...
      _.map( event_map , function( _event ) {
        ...
        obj.dom_el.on( _event.trigger , _event.selector, function( e, event_params ) {
          ...
          //NARROW DOWN THE EVENT SELECTOR TO THE CLICKED ONE
          //what is the clicked element ? @see http://api.jquery.com/event.currentTarget/
          //_actual_event_selector = this;
          //update the _event map
          //_event.selector = this;
          ...
          //control.container.trigger( 'control_changed' , { event : _event } );
        });//.on()
...
(function (api, $, _) {
  $.extend( HUWidgetAreasMethods, {
    initialize: function( id, options ) {
      ...
      //   'control_event_map',
      //   {
      //       trigger   : 'widget_zone_removed',
      //       actions   : ['_fixTopMargin']
      //   }
      // );
      ...
      //bind actions on widget panel expansion
      ...
      //VISIBILITY BASE ON THE SIDEBAR INSIGHTS
    ...
    //overrides the default method to omit the built-in models
    listenToCollectionUpdates : function() {
      var control = this;
      control.container.on('collection_updated', function( e, obj ) {
        //is it an array ?
    ...
    //fired on widget_zone_created
    //scroll to newly created zone
