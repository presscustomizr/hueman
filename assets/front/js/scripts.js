...
(function($, czrapp) {
  var _methods =  {
        ...
        /* ------------------------------------ */
        mobileMenu : function() {
              $('.nav-toggle, .ham__navbar-toggler', '.nav-container' ).on('click', function( ev) {
                    ev.preventDefault();
                    var $navWrap = $(this).siblings('.nav-wrap'),
                        isExpanded = $navWrap.hasClass( 'expanded' );

                    $(this).toggleClass( 'collapsed', isExpanded );
                    $(this).attr('aria-expanded', isExpanded );

                    $.when( $navWrap.toggleClass( 'expanded', ! isExpanded ) ).done( function() {
                          $(this).find('.nav').stop()[ isExpanded ? 'slideUp' : 'slideDown' ]( {
                                duration : 350
                          } );
                    });
